...
import NetFT
import os
from pytesseract_reward import get_page_number, get_previous_page_number
...
class R(RealRobot):
    ...
    def __init__(self):
        ...
        self.flex = False
        self.paperbox = False
        self.hardcode_z = 0.27
        # for flex-flip
        # 0.0045 for training book 0.1mm perpage #0.02 for thick coated book(static) 0.2mm per-page #0.00 for plastic sheets 1mm per-page
    ...
    def get_observation(self, z, ft):
        ...
        ft = [fx, fy, fz, fa, fb, fc]

        return {
            "vector": np.array(ft, dtype=np.float32),
...
# 3
    ...
    def get_reward(self):
        if abs(get_page_number - get_previous_page_number) == 2:
            reward = 1
        else:
            reward = 0
        return reward
